...
All notable changes to Publishing Manager are recorded here using Keep a Changelog categories and Semantic Versioning.
...
- M10 UAT-005: repaired Dashboard launcher-card overflow found during owner UI review. Theme-isolated grid columns, bounded wrapping, readable minimum card height, and 3/2/1-column responsive reflow prevent labels and descriptions from overlapping.

- M10 UAT-006: repaired global-theme fixed button heights that pushed Dashboard summary values and details outside their cards. Multi-line dashboard buttons now explicitly own their box dimensions.

- M10 UAT-007: exclude human-facing Exports and internal System journals from canonical catalog parsing, preventing false schema errors. Diagnostics now foregrounds actionable findings and collapses clear checks.
...
- M8 INT-M-005 through INT-M-007: navigation-only validated `obsidian://publishing-manager` book/edition routes, explicit Settings access toggle and five independently controlled optional field groups, descriptor disclosure of current group state, no source-adapter calls for disabled groups, backward-compatible settings migration, and complete absent/disabled/removed consumer fallback.
...
- Initial Publishing Manager development baseline.
